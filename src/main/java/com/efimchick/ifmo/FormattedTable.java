...
public class FormattedTable {
    ...
    private void addStudentsRows(final StringBuilder builder) {
        Map<Person, Double> totalScores = new Collecting().totalScores(courseResults.stream());
        Map<Person, String> marks = new Collecting().defineMarks(courseResults.stream());
    ...
    private void addLastRow(final StringBuilder builder) {
        ...
        Map<String, Double> taskScores = collecting.averageScoresPerTask(courseResults.stream());
        Object[] formatArgs = new Object[numberOfColumns];
        int index = 0;
        ...
        double totalResult = collecting.averageTotalScore(courseResults.stream());
        formatArgs[index++] = totalResult;
        formatArgs[index] = collecting.defineMark(totalResult);
    ...
    private int defineFirstColumnWidth() {
        return Math.max(MIN_FIRST_COLUMN_WIDTH, 1 + courseResults.stream()
